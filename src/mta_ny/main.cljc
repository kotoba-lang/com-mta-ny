(ns mta-ny.main
  "Kotodama WASM entrypoint for the Mta_ny clean-room actor (L4) — Clojure port.
  ...
  facts (`mta_ny.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mta_ny")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mta_ny.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mta_ny-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mta_ny-compat/ready)
